...
const cerita18 = async () => new Promise((resolve, reject) => {
	const ransex = Math.floor(Math.random() * 2) + 1
	axios.get(`${link}/api/cersex1`)
	.then((res) => {
		resolve(res.data.result.article)
	})
	.catch((err) => {
		reject(err)
	})
...
const video18 = async () => new Promise((resolve, reject) => {
    axios.get(`${link}/api/indohot`)
    .then((res) => {
        resolve(`*Judul:* ${res.data.result.judul}\n*Genre:* ${res.data.result.genre}\n\n*Link!!:* ${res.data.result.url}\n\n*Cara:* Buka linknya, terus tekan download, tunggu 5 detik, tekan download lagi\n*_Note:_* Kalau buka tab baru tutub aja!`)
    })
    .catch((err) => {
        reject(err)
    })
...
const anime = async (anime) => new Promise((resolve, reject) => {
    axios.get(`${link}/api/dewabatch?q=${anime}`)
    .then((res) => {
		if (res.data.error) resolve(res.data.error)
...
const animepict = async (anime) => new Promise((resolve, reject) => {
    axios.get(`${link}/api/dewabatch?q=${anime}`)
    .then((res) => {
		//if (res.data.error) resolve('https://c4.wallpaperflare.com/wallpaper/976/117/318/anime-girls-404-not-found-glowing-eyes-girls-frontline-wallpaper-preview.jpg')
        resolve(`${res.data.thumb}`)
    })
    .catch((err) =>{
        reject(err)
    })
...
const zodiak = async (nama,tanggal) => new Promise((resolve, reject) => {
	axios.get(`${link}/api/getzodiak?nama=${nama}&tgl-bln-thn=${tanggal}`)
	.then((res) => {
		resolve(`Nama: ${res.data.nama}\nLahir: ${res.data.lahir}\nUltah: ${res.data.ultah}\nUsia: ${res.data.usia}\nZodiak: ${res.data.zodiak}`)
	})
	.catch((err) => {
		reject(err)
	})
